document.addEventListener('DOMContentLoaded', () => {
    const API_BASE_URL = 'https://dif-cursos.vercel.app/api/cursos';
    ...
    function setupEventListeners() {
        ...
        btnFiltrosAvancados.addEventListener('click', () => filtrosModal.style.display = 'block');
        ...
        try {
            ...
            modalContent.innerHTML = `
                <div class="curso-detalhes">
                    ...
                    <p><strong>Duração:</strong> ${curso.duracao_horas} horas</p>
                    <p><strong>Preço:</strong> R$ ${curso.preco.toFixed(2)}</p>
                    <p><strong>Avaliação:</strong> ${curso.avaliacao?.toFixed(1) || 'N/A'} / 5</p>
                    <p><strong>Alunos matriculados:</strong> ${curso.alunos_matriculados}</p>
                    <p><strong>Lançamento:</strong> ${new Date(curso.data_lancamento).toLocaleDateString('pt-BR')}</p>
                    <div><strong>Módulos:</strong>
                        <ul>${curso.modulos.map(modulo => `<li>${modulo}</li>`).join('')}</ul>
                    </div>
                </div>
            `;
    ...
    function renderCursos() {
        ...
        cursosContainer.innerHTML = estado.cursos.map(curso => `
            <div class="curso-card">
                <div class="curso-info">
                    <h3 class="curso-titulo" data-id="${curso._id}" style="cursor:pointer;color:#0077cc;text-decoration:underline;">
                        ${curso.titulo}
                    </h3>
                    <p>${curso.duracao_horas}h • ${curso.categoria}</p>
                    <p class="curso-preco">R$ ${curso.preco.toFixed(2)}</p>
                    <div class="curso-acoes">
                        <button class="btn-editar" data-id="${curso._id}">Editar</button>
                        <button class="btn-excluir" data-id="${curso._id}">Excluir</button>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    function abrirFormularioCurso(curso = null) {
        ...
        modalContent.innerHTML = `
            ...
            <form id="form-curso">
                ...
                <div class="form-group">
                    <label>Título:</label>
                    <input type="text" id="curso-titulo" value="${cursoData.titulo}" required>
                </div>
                
                <div class="form-group">
                    <label>Instrutor:</label>
                    <input type="text" id="curso-instrutor" value="${cursoData.instrutor}" required>
                </div>
                
                <div class="form-group">
                    <label>Categoria:</label>
                    <select id="curso-categoria" required>
                        ${["Tecnologia", "Negócios", "Design", "Marketing", "Saúde", "Desenvolvimento Pessoal", "Fotografia", "Música"]
                            .map(opcao => `<option ${cursoData.categoria === opcao ? 'selected' : ''}>${opcao}</option>`).join('')}
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="curso-modulos" required>${cursoData.modulos.join(', ') || ''}</textarea>
                </div>
        ...
        document.getElementById('btn-cancelar').addEventListener('click', () => cursoModal.style.display = 'none');
        cursoModal.style.display = 'block';
    }
    ...
    async function salvarCurso(e) {
        ...
        const cursoData = {
            ...
            avaliacao: parseFloat(document.getElementById('curso-avaliacao').value) || 0,
            alunos_matriculados: parseInt(document.getElementById('curso-alunos').value) || 0,
            data_lancamento: new Date(document.getElementById('curso-data').value).toISOString(),
            modulos: document.getElementById('curso-modulos').value.split(',').map(m => m.trim()),
        };
        ...
        try {
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.errors?.map(e => e.msg).join(', ') || error.message);
            }

            mostrarToast('Sucesso', `Curso ${estado.cursoEditando ? 'atualizado' : 'criado'} com sucesso!`, true);
            cursoModal.style.display = 'none';
    ...
    function atualizarPaginacao() {
        ...
        document.getElementById('paginacao-container').innerHTML = `
            <div class="controles-paginacao">
                ...
                <span class="pagina-atual">Página ${estado.pagina} de ${totalPaginas}</span>
                
                <button class="btn-pagina ${estado.pagina === totalPaginas ? 'disabled' : ''}" 
                    ${estado.pagina === totalPaginas ? 'disabled' : ''} id="btn-proximo">
                    Próxima
                </button>
            </div>
        `;
